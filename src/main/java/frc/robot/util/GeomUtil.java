...
/**
 * Geometry utilities for working with translations, rotations,
...
public class GeomUtil {
  ...
  public static Pose2d inverse(Pose2d pose) {
    ...
    return new Pose2d(pose.getTranslation().unaryMinus().rotateBy(rotationInverse),
        rotationInverse);
  }

  /**
   * Converts a Transform2d to a Pose2d to be used as a position or as
   * the start of a kinematic chain
   *
  ...
  /**
   * Converts a Transform3d to a Pose3d to be used as a position or as
   * the start of a kinematic chain
   *
   * @param transform The transform that will represent the pose
   * @return The resulting pose
  ...
  /**
   * Converts a pose2d to a Pose3d to be used as a position or as the
   * start of a kinematic chain
   *
   * @param pose2d The pose2d that will represent the pose
   * @return The resulting pose
   */
  public static Pose3d toPose3d(Pose2d pose2d) {
    return new Pose3d(pose2d.getTranslation().getX(), pose2d.getY(), 0, Rotation3d.kZero);
  }

  /**
   * Converts a ChassisSpeeds to a Twist2d by extracting two dimensions
   * (Y and Z). chain
   *
  ...
  /**
   * Creates a new pose from an existing one using a different
   * translation value.
   *
   ...
   * @param stepDeg The spacing between allowed angles (degrees).
   * @param offsetDeg The starting offset of the allowed angles
   *        (degrees).
   * @return The snapped rotation.
   */
  public static Rotation2d snapAngle(Rotation2d rot, double stepDeg, double offsetDeg) {
    double snappedDeg = Math.round((rot.getDegrees() - offsetDeg) / stepDeg) * stepDeg + offsetDeg;

    return Rotation2d.fromDegrees(snappedDeg);
  }

  public static Translation2d withY(Translation2d pose, double y) {
    return new Translation2d(pose.getX(), y);
  }
}
