...
/**
 * Manages a chooser value published to the "SmartDashboard" table of
 * NT.
 *
 * @param <V> The value type associated with each string key.
 */
public class ManageableLoggedDashboardChooser<V> extends LoggedNetworkInput {
  ...
  /**
   * Creates a new LoggedDashboardChooser, for handling a chooser input
   * sent via NetworkTables.
   *
   * @param key The key for the chooser, published to
   *        "/SmartDashboard/{key}" for NT or
   *        "/DashboardInputs/SmartDashboard/{key}" when logged.
   */
  public ManageableLoggedDashboardChooser(String key) {
    this.key = key;
    SmartDashboard.putData(key, sendableChooser);
    System.out.println(SmartDashboard.getString(key + "/selected", ""));
    periodic();
  ...
  /**
   * Creates a new LoggedDashboardChooser, for handling a chooser input
   * sent via NetworkTables. This constructor copies the options from a
   * SendableChooser. Note that updates to the original SendableChooser
   * will not affect this object.
   *
   * @param key The key for the chooser, published to
   *        "/SmartDashboard/{key}" for NT or "/DashboardInputs/{key}"
   *        when logged.
   * @param chooser The existing SendableChooser object.
   */
  @SuppressWarnings("unchecked")
  public ManageableLoggedDashboardChooser(String key, SendableChooser<V> chooser) {
    this(key);
    ...
    for (String optionKey : options.keySet()) {
      if (optionKey.equals(defaultString)) {
        setDefaultOption(optionKey, options.get(optionKey));
      } else {
   ...
   * @param value The value of the option.
   */
  public void setDefaultOption(String key, V value) {
    sendableChooser.setDefaultOption(key, key);
    options.put(key, value);
    SmartDashboard.putString(this.key + "/selected", key);
    selectedValue = key;
  }

  public void reloadDefaultOption(String path) {
    String storedDefault = Preferences.getString(path, selectedValue);
    sendableChooser.setDefaultOption(storedDefault, storedDefault);
    SmartDashboard.putString(this.key + "/selected", storedDefault);
  }
   ...
   * There can only be one listener, and this method overrites it with
   * each invokation.
   *
   ...
   * @param listener The function to call that accepts the new value.
   */
  public void onChangeSave(String pathToSave) {
    onChange((v) -> Preferences.setString(pathToSave, selectedValue));
  }

  /**
   * Returns the internal SendableChooser object, for use when setting
   * up dashboard layouts. Do not read data from the SendableChooser
   * directly.
   *
