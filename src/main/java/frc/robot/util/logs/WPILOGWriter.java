...
public class WPILOGWriter implements LogDataReceiver {
    private static final double timestampUpdateDelay = 5.0; // Wait several seconds after DS
                                                            // attached to ensure
    ...
    private static final LoggedNetworkString logNameOverride =
            new LoggedNetworkString("WPILOGWriter/logNameOverride", "");
     ...
     *        AdvantageScope. See
     *        {@link org.littletonrobotics.junction.wpilog.WPILOGWriter.AdvantageScopeOpenBehavior
     ...
     *
     * <p>
     * The logs will be saved to "/U/logs" on the RIO and "logs" in sim.
     * The filename will be generated based on the current time and match
    ...
    public void start() {
        ...
        isOpen = true;
        timestampID = log.start(timestampKey,
    ...
    public void end() {
        ...
        if (shouldOpen) {
            try {
                String fullLogPath = FileSystems.getDefault().getPath(folder, filename).normalize()
                ...
                DriverStation.reportError("[AdvantageKit] Failed to send log to AdvantageScope.",
                        false);
            }
        }
    }
        ...
        if (autoRename) {
            ...
            if (logDate == null) {
                if ((table.get("DriverStation/DSAttached", false)
                    ...
                    } else if (RobotController.getFPGATime() / 1000000.0
                            - dsAttachedTime > timestampUpdateDelay || RobotBase.isSimulation()) {
            ...
            String eventNameOverride = logNameOverride.get();
            if (eventName.isEmpty() && !eventNameOverride.isEmpty()) {
            ...
            if (!newFilename.equals(filename)) {
                ...
                System.out.println("[AdvantageKit] renamed log to \"" + logPath + "\"");
            }
        }
            ...
            if (appendData) {
                ...
                switch (field.getValue().type) {
                    ...
                    case String:
                        ...
                        break;
                    case BooleanArray:
                        log.appendBooleanArray(id,
                                field.getValue().getBooleanArray(),
                                table.getTimestamp());
                        break;
                    case IntegerArray:
                        log.appendIntegerArray(id,
                                field.getValue().getIntegerArray(),
                                table.getTimestamp());
                        break;
                    case FloatArray:
                        log.appendFloatArray(id,
                                field.getValue().getFloatArray(),
                                table.getTimestamp());
                        break;
                    case DoubleArray:
                        log.appendDoubleArray(id,
                                field.getValue().getDoubleArray(),
                                table.getTimestamp());
                        break;
                    case StringArray:
                        log.appendStringArray(id,
                                field.getValue().getStringArray(),
                                table.getTimestamp());
                        break;
                }
            }
    ...
    /**
     * The behavior to use when sending the log file to AdvantageScope
     * after the robot program exits in simulation.
     */
